...
#### 1. 大模型 Block 效果优化
- 研究 @xenova/transformers 的 temperature/top-k/top-p 参数调优
- 测试不同模型（sentiment-analysis vs 专用中文情感模型）
- 添加 `mlTemperature`、`mlTopP` 等配置项
- 实现 100ms 超时回退机制

#### 2. 闭包运算：动态黑名单扩充
**核心思路**：Block 用户时，自动提取该用户评论中的关键词，智能添加到黑名单

用户点击 "屏蔽用户" 后的流程：
```
Block User → 获取该用户历史评论 → 分词/关键词提取
→ 用户确认 → 添加到黑名单词库
```

- 支持用户主动勾选："将评论关键词也加入屏蔽"
- 避免误纳：只添加高频出现的 pattern，而非单个词
- 可在设置中开启/关闭此功能

#### 3. 导入/导出方案研究
- 调研 chrome.storage API 的导入导出限制
- 支持 JSON 格式的配置与黑名单互导
- 考虑与 bilibili-cleaner 等项目的格式兼容
- 支持规则包导入（其他用户分享的配置）
...
| 功能 | 描述 |
|------|-------|
| 用户画像分析 | 基于评论历史分析用户类型（营销号/杠精/正常用户） |
| 智能阈值 | 根据社区整体质量动态调整过滤阈值 |
| 跨设备同步 | chrome.storage.sync 同步黑名单与设置 |
| 批量 Block | 支持选择多个用户/评论批量屏蔽 |
...
|------|-------|
| 分布式规则库 | 用户可订阅/分享屏蔽规则 |
| 本地学习 | 基于用户反馈（"仍然显示"）持续优化本地模型 |
| API 扩展 | 支持对接第三方 API（更精准的中文 NLP） |
